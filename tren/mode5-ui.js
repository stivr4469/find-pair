...
function displayMode5QuestionUI(question) {
    ...
    contentArea.innerHTML = `
        <div class="question-container view-enter" style="padding-bottom: 80px;"> <div class="progress-text">Выполнено ${mode5State.totalAnswered} из ${mode5State.sessionLimit}</div> <div class="context-box" style="background: var(--accent-faint); padding: 10px; border-radius: 5px; margin-bottom: 10px; font-style: italic; color: var(--accent); border: 1px solid var(--accent-mid);"> <strong>Контекст:</strong> ${contextText}
                ...
                <button id="speak-btn" style="background: none; border: none; cursor: pointer; margin-left: 10px; display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:50%;background:var(--faint);color:var(--text);" title="Озвучить" onclick="playCurrentMode5Phrase()"></button> </div> <div class="options-container stagger" id="mode5-options" style="display: grid; gap: 10px;"> ${question.options.map(option => `
