...
function displayMode4QuestionUI(question) {
    ...
    contentArea.innerHTML = `
        <div class="question-container view-enter" style="padding-bottom: 80px;"> <div class="progress-text">Вопрос ${mode4State.currentQuestionIndex + 1} из ${mode4State.totalQuestions}</div> <div class="question-text">${question.russian}</div> <div class="options-container stagger" id="mode4-options"> ${shuffledOptions.map(option => `
    ...
    contentArea.querySelectorAll('.option-btn').forEach(button => {
        button.addEventListener('click', () => {
            if (!mode4State.isAnswered) {
                checkMode4Answer(button.dataset.answer, question.spanish, button, question.explanation);
            }
        });
    });
...
function showMode4ResultsUI() {
    ...
    contentArea.innerHTML = `
        <div class="results-container view-enter"> <h3>Результаты</h3> <div class="final-score">${mode4State.score} из ${mode4State.totalQuestions} (${percentage}%)</div> <div class="final-message">${message}</div> <button class="restart-button" onclick="restartMode4()">Ещё раз</button> <button class="menu-button" onclick="showMainMenu()">Меню</button> </div> `;
}
