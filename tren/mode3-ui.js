...
function displayMode3QuestionUI(question) {
    ...
    contentArea.innerHTML = `
        <div class="question-container view-enter" style="padding-bottom: 80px;"> <div class="progress-text">Вопрос ${mode3State.currentQuestionIndex + 1} из ${mode3State.totalQuestions}</div> <div class="translation-text">${question.translation}</div> <div class="question-text">${question.text.replace('___', '<span class="blank" id="question-blank">___</span>')}</div> <div class="options-container stagger" id="mode3-options"> ${options.map(option => `
...
function showMode3ResultsUI() {
    ...
    contentArea.innerHTML = `
        <div class="results-container view-enter"> <h3>Результаты</h3> <div class="final-score">${mode3State.score} из ${mode3State.totalQuestions} (${percentage}%)</div> <div class="final-message">${message}</div> <button class="restart-button" onclick="restartMode3()">Ещё раз</button> <button class="menu-button" onclick="showMainMenu()">Меню</button> </div> `;
}
