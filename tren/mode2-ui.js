...
function displayAdvancedSettings_tren() {
    ...
    contentArea.innerHTML = `
        <div class="settings-container view-enter"> <h3>Настройки тренировки (A2)</h3> <div class="setting-group"> <h4>Глаголы:</h4> <div id="verbs-settings-tren"> ${verbs.map(verb => `<label><input type="checkbox" name="verb" value="${verb}" checked> ${verb}</label>`).join('')}
                </div> </div> <div class="setting-group"> <h4>Времена:</h4> <div id="tenses-settings-tren"> ${Object.keys(tenses).map(tense => `<label><input type="checkbox" name="tense" value="${tense}" checked> ${tenses[tense]}</label>`).join('')}
                </div> </div> <button class="start-button" id="start-tren-mode2-btn">Начать тренировку</button> </div> `;
...
function displayAdvancedQuestion_tren(question, options) {
    ...
    contentArea.innerHTML = `
        <div class="question-container view-enter" style="padding-bottom: 80px;"> <div class="progress-text">Вопрос ${advancedModeState_tren.questionCount + 1} из ${advancedModeState_tren.maxQuestions}</div> <button class="back-to-settings-button" onclick="initMode2()">К настройкам</button> <div class="question-text">${question.questionText}</div> <div class="options-container stagger" id="mode2-options"> ${options.map(option => `<button class="option-btn" data-answer="${option}">${option}</button>`).join('')}
            </div> <div class="feedback" id="mode2-feedback"></div> <button class="next-button quiz-next-fixed" id="mode2-next-btn" style="display: none;">Дальше →</button> </div> `;
...
function showAdvancedResults_tren() {
    ...
    contentArea.innerHTML = `
        <div class="results-container view-enter"> <h3>Результаты</h3> <div class="final-score">${advancedModeState_tren.score} из ${advancedModeState_tren.maxQuestions} (${percentage}%)</div> <div class="final-message">${message}</div> <button class="restart-button" onclick="initMode2()">Ещё раз</button> <button class="menu-button" onclick="showMainMenu()">Меню</button> </div> `;
}
